...
<div style="background-color: #333333" class="super_container">
	...
	<div style="background-color: #333333" class="product_details">
		<div style="background-color: #333333" class="container">
			<div style="background-color: #333333" class="row details_row">
				...
				<div class="col-lg-6">
					<div class="details_content">
						<div class="details_name text-light">{{$products->product_name}}</div>
						@php
							$home = new Home;
							$harga = $home->diskon($products->discount,$products->price);
						@endphp
						@if ($harga != 0)
							<div class="details_discount text-light">Rp.{{number_format($products->price)}}</div>
							<div class="details_price text-light">Rp.{{number_format($harga)}}</div>
						@else
							<div class="details_price text-light">Rp.{{number_format($products->price)}}</div>
						@endif
						<!-- In Stock -->
						<div class="in_stock_container">
							<div class="availability text-light">Availability:</div>
							@if ($products->stock <= 0)
								<span class="text-light" style="color:red;">Out of Stock!</span>
							  @else
							  	@if ($products->stock <= 5) 
									<span class="text-light" style="color:red;">Hurry Up!</span>
									<p class="text-light" style="color:black;">Only {{$products->stock}} left!</p>
								@else
									<span class="text-light">In Stock</span>
									<p class="text-light" style="color:black;">{{$products->stock}} left</p>
								@endif
              				@endif
						</div>
			...
			<section id="reviews" class="pb-5 mt-4">
				...
				<h4 class="h4-responsive dark-grey-text font-weight-bold my-5 text-center">
				  ...
				  <strong class="text-light">Product Reviews</strong>
				...
				<div class="comments-list text-center text-md-left">
				  @if (!$products->product_review->count())
					<div class="d-flex justify-content-center">    
					  <div class="row mb-5">
						   <p><strong class="text-light">Belum ada review produk.</strong></p> 
					  </div>
					</div>
				  @else
						...
						<div class="col-sm-10 col-12">
						  ...
						  <a>
							{{-- @php
								dd(Auth::user()->id);
							@endphp --}}
							<h5 class="user-name font-weight-bold text-light">{{$item->user->name}} 
							</h5>
						  ...
						  <p class="text-light article">{{$item->content}}</p>
							...
							<div class="row mb-5" style="margin-left: 5%">
							  ...
							  <div class="col-sm-10 col-12">
								...
								<a>
								  ...
								  <h5 class="user-name font-weight-bold text-light"><span style="margin-right:5px;" class="badge success-color">Admin</span>{{$balasan->admin->name}}</h5>
								...
								<p class="text-light article">{{$balasan->content}}</p>
